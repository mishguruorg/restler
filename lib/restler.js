...
function Request(uri, options) {
  ...
  if(options.headers['Accept-Encoding'] !== false) {
  ...
  if (this.options.multipart) {
    ...
    if (typeof this.options.data == 'object' && !Buffer.isBuffer(this.options.data)) {
      ...
      this.headers['Content-Type'] = options.headers['Content-Type'] || 'application/x-www-form-urlencoded';
  ...
  if (this.options.excludeContentLengthHeader) {
    delete this.headers['Content-Length'];
  }

  var proto = this.url.protocol === 'https:' ? https : http;
  if (this.url.protocol === 'https:' && this.options.proxy) {
    var proxy = 'http://' + this.options.proxy.host + ':' + this.options.proxy.port
    this.request = proto.request({
      host: this.url.hostname,
      port: this.url.port,
      path: this.options.proxy ? this.url.href : this._fullPath(),
    ...
    this.request = proto.request({
      host: this.options.proxy ? this.options.proxy.hostname : this.url.hostname,
      port: this.options.proxy ? this.options.proxy.port : this.url.port,
      path: this.options.proxy ? this.url.href : this._fullPath(),
      method: this.options.method,
      headers: this.headers,
      rejectUnauthorized: this.options.rejectUnauthorized,
